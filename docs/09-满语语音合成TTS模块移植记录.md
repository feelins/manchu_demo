...
## 5.1 输入规范：传统满文必须先转写为拉丁

**结论：合成引擎只吃拉丁满文，喂传统满文会静默产出空音频。** 这是本模块最容易踩的坑，单列一节。

### 现象（实测）

| 输入 | 上游返回 |
|---|---|
| `ᠰᡳ ᠶᠠᠪᠠᡩᡝ ᡨᡳᠶᡝᡥᡝᠪᡳ`（传统满文） | `"cmudict_tokens": []`，`latin_text` 原样返回满文 → **空音素，合成结果是空音频，且不报错** |
| `si yabade tiyehebi`（拉丁） | 正常 15 个音素 |

### 原因

`GPT_SoVITS/text/manchu.py` 里：

- `text_normalize()` **只做两件事**：中文标点 → 英文标点、转小写，**不做满文转拉丁**；
- `g2p()` 的分词正则是 `_WORD_TOKEN = r"[a-z']+|[.!?,…-]"`，**只匹配小写拉丁字母和撇号**，满文字符被整段跳过，于是一个词都进不了词典，返回空列表。

词典 `manchu_cmudict.txt`（48,230 条）的 key 是拉丁拼写，含 `g'abixara`、`k'...` 等撇号形式，**与门户 `manju-rules.js` 的输出规范一致**，所以规则转出来的拉丁可以直接用。

### 处理办法（两端各一道）

1. **前端自动转写**（`static/js/tts.js`）：检测到满文字符（U+1800–U+18AF / U+202F / U+200D）时，先调公共规则库的
   `ManjuRules.manju2latin()` 转成拉丁再送上游，并在日志区写明"已自动转写为拉丁"。
   左侧竖排预览改为显示「规范化拉丁 → 回满文」的镜像，正好可肉眼核对转写是否走样。
2. **后端兜底拒绝**（`modules/tts/__init__.py`）：`/api/tts/{convert,synthesize}` 检测输入含满文字符时返回 **400 + 明确提示**，
   避免 curl / 脚本等调用方拿到空音素却不知原因。

> 为什么后端不"顺便代转"？—— 满文⇄拉丁规则全站只有一份实现（`static/js/manju-rules.js`）。
> 在后端代转等于再抄一份 Python 规则，违反该约定，且两份规则迟早会不一致。故后端只拦不转。

### 验证（全链路）

```
① 满文原文 : ᠰᡳ ᠶᠠᠪᠠᡩᡝ ᡨᡳᠶᡝᡥᡝᠪᡳ
② 规则转写 : si yabade tiyehebi
③ 音素     : S IY1 JH AW1 P AW1 T AH1 T JH AE1 HH AH1 P IY1   ← 与手工输入拉丁完全一致
④ 合成产物 : /api/tts/media/manchu_a643898d66d2.wav（131884 字节，约 2.7 秒）
⑤ 门户回源 : 200，字节正常
```
...
- [ ] **转写覆盖率核对**：`manju-rules.js` 的满文→拉丁输出与 TTS 词典（48,230 条）key 拼写逐类比对（尤其 `ng` / `k'` / `si'` / 双元音 `oo→ou` 等），统计 OOV 率
- [ ] **右侧可切换输入模式**：目前自动判定，后续可加"满文 / 拉丁"显式切换与转写对照高亮
